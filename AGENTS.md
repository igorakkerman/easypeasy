...
- Elevation: a write only administrators may perform — `-Machine` on the Path and environment functions, `-AllUsers` on the Start Menu ones, `-Elevated` on `Register-LogonTask` (a task registered at the logon privileges of the current user needs no elevation) — re-runs **the whole command** through `Invoke-Elevated` (from `elevate.ps1`) when the session is not already elevated, then syncs whatever the parent process caches. One prompt per command, never one per write, and the read-modify-write stays inside the elevated session rather than straddling it. `local:ConvertTo-ElevatedCommand` rebuilds the command line from `$PSBoundParameters` where a function has more than a couple of parameters to pass on. `Register-LogonTask -Elevated` takes the other shape: it builds the task in-process, which needs no privilege, and elevates `Register-ScheduledTask -Xml` alone — **the elevated session runs a built-in command, not a module one**, so the registration no longer depends on the child resolving `easypeasy` on its own `PSModulePath`. Prefer this wherever the privileged step is a single primitive fed by an object the module can serialize. The exception is `Set-`/`Remove-EnvironmentVariable`, the primitive the Path functions write through: it elevates its own single write. A command whose run will elevate calls `local:Assert-SudoAvailable` first — as early as the elevation is known, before any read, prompt or write — so sudo missing from the system, switched off in Settings or by group policy, or capped below the inline mode the elevation runs in, fails the command up front rather than halfway through. `Invoke-Elevated` asserts again before it runs sudo.
