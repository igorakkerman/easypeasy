...
- Elevation: a write to a scope only administrators may write — `-Machine` on the Path and environment functions, `-AllUsers` on the Start Menu ones — re-runs **the whole command** through `Invoke-Elevated` (from `elevate.ps1`) when the session is not already elevated, then syncs whatever the parent process caches. One prompt per command, never one per write, and the read-modify-write stays inside the elevated session rather than straddling it. `local:ConvertTo-ElevatedCommand` rebuilds the command line from `$PSBoundParameters` where a function has more than a couple of parameters to pass on. The exception is `Set-`/`Remove-EnvironmentVariable`, the primitive the Path functions write through: it elevates its own single write. A command whose run will elevate calls `local:Assert-SudoAvailable` first — as early as the elevation is known, before any read, prompt or write — so sudo missing from the system, switched off in Settings or by group policy, or capped below the inline mode the elevation runs in, fails the command up front rather than halfway through. `Invoke-Elevated` asserts again before it runs sudo.
...
- **Unresolved `%...%` references never fail a command.** `local:Get-UnresolvedVariableName` names the references a value carries that the process environment does not set; `ConvertTo-NormalizedLocation` returns `$null` for such a value rather than prefixing the current directory to it, and `local:ConvertTo-LocationIdentity` falls back to the comparable stored value, so an unresolved reference is matched, selected and deduplicated by the reference itself. `local:Write-UnresolvedVariableError` reports one non-terminating error per missing variable, and only at the public boundary: a command reports the references of the location or value it was given, a read reports those of the entries it lists, and the reads a write command makes on its way are silenced with `-ErrorAction SilentlyContinue` so a write speaks about its argument alone.
