...
- **Parameter attributes**: never write `Mandatory = $false` — it is the default. Where it is all the attribute holds, drop the whole `[Parameter()]` line; where the attribute also carries `ParameterSetName` or the like, keep that and drop only the `Mandatory`. Do not add `Position` to make a parameter positional, and do not add `PositionalBinding` to `[CmdletBinding()]`: whatever position PowerShell derives from declaration order is accepted, and is not a contract to spell out. The `Position` values already in the module stay — a command with several parameter sets is not positional without them, and those calls are documented and tested.
