...
- **Machine / User / Effective parameter-set pattern**: read functions offer `-Machine`, `-User`, and a default `-Effective` (current-process value); write functions offer `-Machine` and `-User` (the default). Inside its `ShouldProcess` block a write checks `local:Test-Elevated`; a `-Machine` write that is not already elevated calls `Invoke-Elevated <self> <args>`, which runs the command in an elevated process (`Start-Process -Verb RunAs`, no `sudo`) instead of writing in-process. Either way the parent syncs its own process afterward.
