...
- Path edits keep the current process (`$env:PATH`) and the persisted registry value in sync; location comparisons go through `local:Get-LocationKey`, which ignores repeated and trailing backslashes.
