...
- Layering: `systempath.ps1` builds on `environment.ps1` (Path is just an env var); `startmenu.ps1` builds on `shortcut.ps1`, calling `Set-ShortcutRunAsAdministrator` and the `ShortcutWindowStyle` enum; a `-Machine` write that is not already elevated re-runs itself through `Invoke-Elevated` (from `elevate.ps1`). `Assert-Elevated` stays exported for callers that want a hard admin check.
